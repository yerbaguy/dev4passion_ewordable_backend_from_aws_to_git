...
def hello():
   ...
   for value in data:
       try:
           for key, val in value.itms():
               if key == "text":
                   transcript += value
       except: 
           print(f"{val} doesn't have any transcript")
...
@app.route('/get_wordd', methods = ['GET'])
def get_a_word_from_firebase1():
    ...
    if random_word > 10:
        ##print("random_word_is", random_word)
        x = random_word
        n = 3
        #for doc_data4 in random_word[{x}:5]:
        #for doc_data4 in random_word:
        for j in range(0, n + 1):
            x += j
            ##print(x)
            #print("doc_data2", doc_data2["definition"]["definitions"][0]["definition"])
            ##print("doc_data2", documents_list[x]["definition"]["definitions"][0]["definition"])
            document_list_data = documents_list[x]["definition"]["definitions"][0]["definition"]
            random_word_left.append(document_list_data)
            ####for document in document_list_data:
            ####    document_data = document.to_dict()
            ####    random_word_left.append(document_data)
            
            ##random_word_left.append(document_list_data)
            #document_list_dataa = document_list_data.to_dict()
            #random_word_left.append(document_list_dataa)
    elif random_word < len(documents_list) - 10:
         #amount = len(documents_list) - 10
         print("no", random_word)
         #print("amount", amount)


    #return {"hi":"hi"}
    #return doc_data
    return [doc_data, random_word_left]
...
def get_a_word_from_firebase():
    ...
    #if random_word > 0 and random_word < len(documents_list):
    #    print("random_word_is", random_word)
    #else:
    #    print("no", random_word)
    ...
    #random_word_left = rand_word_left(doc_data_len)
    random_word_left = rand_wordd(doc_data_len)
    print("random_word_left", random_word_left)
...
def rand_wordd(a):
    random_word_left = random.randint(0, a - 1)
    #if random_word_left > 4:
    #    rand_word_left -= 4;
    return random_word_left
